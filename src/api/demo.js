...
function decryptImage (url_, this_, callback) {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      let key = [118, 111, 100, 95, 112, 114, 111, 106, 101, 99, 116, 95, 101, 110, 99, 101]
...
function toObtain () {
  ...
  if (isAndroid) {
    document.querySelector('.android').setAttribute('style', 'display:block;')
    ...
    if (width_ >= 1024) {
      ...
      document.querySelector('.col-lg-4').setAttribute('style', `"margin":"0 auto","position": "absolute","left": "50%","transform": "translate(-50%)"`)
    }
  } else {
    document.querySelector('.android').setAttribute('style', 'display:block;')
    imgLen = document.querySelectorAll('.android img')
