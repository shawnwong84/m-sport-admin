<template>
    <div class="info-table">
        ...
        >
            ...
            <el-table-column prop="remark" label="活动内容"> </el-table-column>
            <el-table-column prop="sysPicUrl" label="跳转链接">
            </el-table-column>
        ...
        >
            <el-form :model="form" ref="form" :rules="rules">
                ...
                >
                    ...
                    <div
                        v-if="
                            isPicPath === '2' ||
                            (!isAdd && !isCopy && !isDelete)
                        "
                    >
                        <img
                            v-if="form.base64List"
                            :src="form.base64List"
                            width="100px"
                        />
                        ...
                        <el-upload
                            ...
                            v-if="isAdd"
                        >
                            <el-button size="small" type="primary" v-if="isAdd"
                                >点击上传</el-button
                            >
                ...
                <el-form-item
                    ...
                    :label-width="formLabelWidth"
                    prop="navigateName"
                    v-if="!isCopy && !isDelete && pronavigateList.length !== 0"
                >
                    <el-radio
                        v-for="(item, index) in pronavigateList"
                        :key="index"
                        v-model="navigateName"
                        :label="item.navigateName"
                        >{{ item.navigateName }}</el-radio
                    >
                </el-form-item>
                <el-form-item
                    label="渠道"
                ...
                >
                    ...
                    <el-checkbox-group v-model="checkId">
                        <el-checkbox
                            v-for="(item, index) in channelDate"
                            :key="index"
                            :label="item"
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button
                    type="primary"
                    v-if="!isAdd && !isCopy && !isDelete"
                    @click="updateSumbit"
                    >确 定</el-button
                >
                <el-button type="primary" v-if="isCopy" @click="copySumbit"
                    >确 定</el-button
                >
                <el-button type="primary" v-if="isDelete" @click="deleteSumbit"
                    >确 定</el-button
                >
            </div>
        </el-dialog>
    ...
    methods: {
                ...
                if (res.code === '0') {
                    ...
                    for (var i = 0; i < this.tableData.length; i++) {
                        if (this.tableData[i].picPath.substr(-3, 3) === '.en') {
                            this.decryptImage(
                                this.imgUrl + this.tableData[i].picPath,
                                this.tableData[i],
                            );
                        }
                    }
        ...
        addImg() {
            ...
            for (let i = 0; i < this.pronavigateList.length; i++) {
                if (
                    this.navigateName === this.pronavigateList[i].navigateName
                ) {
                    this.$set(
                        this.form,
                        `bpromoteNavigate`,
                        this.pronavigateList[i],
                    );
                }
            }
        ...
        copySumbit() {
            ...
            if (JSON.stringify(this.form.channelMap) !== '{}') {
                ...
                this.$axios('post', '/promote/copyPronote', param).then(
                    (res) => {
                        if (res.code === '0') {
                            this.$message({
                                type: 'success',
