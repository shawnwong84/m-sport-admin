<template>
    <div class="info-table">
        ...
        >
            ...
            <el-table-column label="身份证正面图">
                <template slot-scope="scope">
                    <img
                        v-if="scope.row.cardPhotoPositive"
                        :src="scope.row.cardPhotoPositive"
                        width="100px"
                    />
                </template>
            </el-table-column>
            <el-table-column label="身份证反面图">
                <template slot-scope="scope">
                    <img
                        v-if="scope.row.cardPhotoReverse"
                        :src="scope.row.cardPhotoReverse"
                        width="100px"
                    />
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        editUserApply(userId, status) {
            ...
            this.$axios('post', '/backUser/editUserApply', param).then(
                (res) => {
                    if (res.code === 200) {
                        this.$message({
                            type: 'success',
        ...
        getInfo(num) {
            ...
            this.$axios('post', '/backUser/userApplyAnchorList', param).then(
                (res) => {
                    if (res.code === 200) {
