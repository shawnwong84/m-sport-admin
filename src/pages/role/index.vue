    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span v-for="i in scope.row.menus" class="mr-10">{{i}}</span>
        </template>
      </el-table-column>
      <el-table-column
        fixed="right">
    ...
    <el-dialog title="角色管理" :visible.sync="dialogFormVisible"  width="388px">
      <el-form :model="form" ref="form" :rules="rules">
        <el-form-item label="角色名称" :label-width="formLabelWidth" prop="roleName">
          <el-input v-model="form.roleName" autocomplete="off" style="width: 217px;"></el-input>
        </el-form-item>
        <el-form-item label="角色描述" :label-width="formLabelWidth" prop="roleDesc">
          <el-input v-model="form.roleDesc" autocomplete="off" style="width: 217px;"></el-input>
        </el-form-item>
        <el-form-item label="可操作栏目" :label-width="formLabelWidth">
          <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">全选</el-checkbox>
          <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange">
            <el-checkbox v-for="(item,index) in menuList" :label="item" :key="index">{{item}}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
      </el-form>
        ...
        if (this.checkedCities) {
          ...
          for (var i = 0; i < this.checkedCities.length; i++) {
            for (var k = 0; k < this.menuListData.length; k++) {
              if (this.checkedCities[i] === this.menuListData[k].name) {
                mid.push(this.menuListData[k].id)
                const source = JSON.parse(`{"${this.menuListData[k].id}":"${this.menuListData[k].name}"}`)// 利用JSON.parse将对象
      ...
      updateInfo () {
        ...
        this.$axios('post', '/sys/role/updateRole', param).then((res) => {
          if (res.code === 200) {
            ...
            this.getInfo()
            if (this.form.id === parseInt(sessionStorage.getItem('roleId'))) {
              location.reload()
            }
            this.clear()
          }
