...
<!--        </div>-->
    ...
    <el-table
      :data="tableData.filter(data => !search || data.version.toLowerCase().includes(search.toLowerCase()))"
      style="width: 100%;"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span v-if="scope.row.device === 1" class="c-green">Android</span>
          <span v-if="scope.row.device === 2" class="c-red">ios</span>
          <span v-if="scope.row.device === 3" class="c-red">其他系统</span>
        </template>
      </el-table-column>
      <el-table-column
        prop="channelName"
      ...
      <el-table-column
        ...
        label="是否强制更新">
        <template slot-scope="scope">
          <span v-if="scope.row.isForce === 1" class="c-green">是</span>
...
<!--          <span v-if="scope.row.device !== 2">{{apkUrl+scope.row.adress}}</span>-->
      ...
      <el-table-column
        ...
        <template slot-scope="scope" class="cursor">
          <span class=" down-btn el-icon-download" @click="download(scope.row.adress,scope.row.device)"></span>
        </template>
      </el-table-column>
      <el-table-column
        fixed="right">
    ...
    <el-dialog title="下载管理管理" :visible.sync="dialogFormVisible"  width="400px">
      <el-form :model="form" ref="form" :rules="rules">
        <el-form-item label="设备" :label-width="formLabelWidth" prop="device">
          <el-radio @change="changeDevice(1)" v-model="form.device" :label="1">Android</el-radio>
          <el-radio @change="changeDevice(2)" v-model="form.device" :label="2">ios</el-radio>
          <el-radio @change="changeDevice(3)" v-model="form.device" :label="3">其他系统</el-radio>
        </el-form-item>
        <el-form-item label="版本" :label-width="formLabelWidth" prop="version" >
          <el-input  v-model="form.version" autocomplete="off" style="width: 238px;"></el-input>
        </el-form-item>
        <el-form-item label="是否强制更新" :label-width="formLabelWidth" prop="isForce" >
...
<!--        </el-form-item>-->
        <el-form-item label="描述" :label-width="formLabelWidth" prop="descr" >
          <el-input v-model="form.descr"  autocomplete="off" style="width: 238px;margin-right: 10px;"></el-input>
        </el-form-item>
...
<!--        </el-form-item>-->
        <el-form-item label="请选择" :label-width="formLabelWidth" v-if="form.device !== 2 && isAdd">
          <el-radio  v-model="isFile" :label="1">上传app包</el-radio>
          <el-radio  v-model="isFile" :label="2">输入app包地址</el-radio>
        </el-form-item>
        <el-form-item label="上传app包" :label-width="formLabelWidth" v-if="form.device !== 2 && isFile === 1  && isAdd">
<!--          http://hszhibo.live/-->
          <el-upload
            class="upload-demo"
        ...
        <el-form-item label="下载地址" :label-width="formLabelWidth" prop="adress" v-if="((form.device === 2  || isFile === 2)  && isAdd) || (!isAdd && !isDelete)">
          <el-input  type="textarea"
                     :rows="4" v-model="form.adress" :disabled="isChannel" autocomplete="off"  style="width: 238px;margin-right: 10px;"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="clear">取 消</el-button>
        <el-button type="primary" v-if="isAdd" @click="addSumbit">确 定</el-button>
        <el-button type="primary" v-if="!isAdd && !isDelete" @click="updateSumbit">确 定</el-button>
...
export default {
    ...
    methods: {
        ...
        this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            if (this.form.adress === '') {
              ...
              let param = this.form
              this.$axios('post', '/download/save', param).then((res) => {
                if (res.code === 200) {
                  this.$message({
                    type: 'success',
                    message: '新增成功!'
      ...
      updateInfo () {
        this.addFile()
        let param = this.form
        this.$axios('post', '/download/updateDownLoad', param).then((res) => {
          if (res.code === 200) {
            this.$message({
              type: 'success',
              message: '操作成功!'
