<template>
  <div class="info-table">
    <div class="fx align-items justify-between mt-15 mb-15 ml-10 mr-10">
      <div class="fs-18 fw-600">公告管理</div>
    ...
    <el-dialog title="公告管理" :visible.sync="dialogFormVisible"  width="490px">
      <el-form :model="form" ref="form" :rules="rules">
        <el-form-item label="公告文案" :label-width="formLabelWidth" prop="content">
          <el-input
            type="textarea"
            :rows="10"
            placeholder="请输入内容"
  ...
  export default {
    ...
    data () {
      return {
        ...
        rules: {
          fakeMsg: [
            { required: true, message: '请输入公告文案', trigger: 'blur' }
          ]
        }
      }
    },
    ...
    methods: {
      ...
      getInfo () {
        ...
        this.$axios('post', '/backAnnouncement/getList', param).then((res) => {
          if (res.code === 200) {
      ...
      addSumbit () {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            let param = this.form
            this.$axios('post', '/backAnnouncement/editAnnouncement', param).then((res) => {
              if (res.code === 200) {
                this.$message({
                  type: 'success',
                  message: '新增成功!'
