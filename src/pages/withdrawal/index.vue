  ...
  <div class="info-table">
    <div class="fx align-items justify-between mt-15 mb-15 ml-10 mr-10">
      ...
      <div class="fx align-items">
        <div><el-input
          placeholder="输入提款账号名"
          size="small"
          v-model="cardName" class="w-150">
        </el-input></div>
        <div class="ml-10"><el-input
          placeholder="输入用户名"
          size="small"
          v-model="userName" class="w-150">
        </el-input></div>
        <div class="ml-10">
          <el-input
            placeholder="输入提款金额"
        ...
        <div class="ml-10">
          <el-select v-model="status" placeholder="请选择提款状态" size="small" class="w-150">
        ...
        <div class="ml-10"><el-button type="primary"  size="small" icon="el-icon-plus" @click="getInfo">查询</el-button></div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        width="200">
        <template slot-scope="scope">
          <el-button v-if="scope.row.status === 0" type="primary" class="cursor" size="mini" plain
                     @click="comission(scope.row.id,1)">通过</el-button>
          <el-button v-if="scope.row.status === 0" type="primary" class="cursor" size="mini" plain
                     @click="comission(scope.row.id,-1)">拒绝</el-button>
          <el-button v-if="scope.row.status === 1" type="primary" class="cursor" size="mini" plain
                     @click="comission(scope.row.id,2)">确认成功</el-button>
          <el-button v-if="scope.row.status === 1" type="primary" class="cursor" size="mini" plain
                     @click="comission(scope.row.id,-1)">转账失败</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    getInfo () {
      ...
      this.$axios('post', '/comission/withdrawList?pageNo=' + this.ps.pageNo + '&pageSize=' + this.ps.pageSize, param).then((res) => {
        if (res.code === '0') {
    ...
    comission (id, status) {
      this.$confirm('确定进行该操作？', '提示', {
        ...
        this.$axios('post', '/comission/update?id=' + id + '&status=' + status).then((res) => {
          if (res.code === '0') {
            this.$message({
              type: 'success',
              message: '操作成功!'
