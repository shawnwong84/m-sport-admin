...
<!--          <span v-if="scope.row.adType === '3'">电影</span>-->
            ...
            <div class="ml-10">
              <div  @click="openLiveConfig(scope.row.id)">
                <el-button type="primary" class="cursor"
                           size="mini" plain >新增信号源</el-button>
              </div>
              <div class="mt-10"  @click="updateConfig(scope.row.id)">
                <el-button type="primary" class="cursor"
                           size="mini" plain >修改信号源</el-button>
              </div>
            </div>
          </div>
        </template>
    ...
    <el-dialog title="赛事管理" :visible.sync="dialogFormVisible"  width="550px">
      <el-form :model="form" ref="form">
        <el-form-item label="主队" :label-width="formLabelWidth"  prop="homeId" >
          {{homeName}}
          <el-button type="primary"  size="small" @click="openTeam(1)">选择</el-button>
        </el-form-item>
        <el-form-item label="主队得分" :label-width="formLabelWidth" prop="homeSource">
          <el-input v-model.number="form.homeSource"  style="width: 200px;margin-right: 10px;"></el-input>
        </el-form-item>
        <el-form-item label="客队" :label-width="formLabelWidth"  prop="awayId" >
          {{awayName}}
          <el-button type="primary"  size="small" @click="openTeam(2)">选择</el-button>
        </el-form-item>
        <el-form-item label="客队得分" :label-width="formLabelWidth" prop="awaySource">
          <el-input v-model.number="form.awaySource"  style="width: 200px;margin-right: 10px;"></el-input>
        </el-form-item>
        <el-form-item label="赛程类型" :label-width="formLabelWidth"  prop="sportsId" >
          <el-select v-model="form.sportsId" placeholder="请选择">
            <el-option
                v-for="(item,index) in sportsList"
                :key="index"
                :label="'['+ item.sportsName+']'+ item.competitionName"
                :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
    ...
    <el-dialog title="队伍列表" :visible.sync="dialogFormVisible1"  width="80%">
      <el-table
          :data="teamList"
          style="width: 100%;"
    ...
    <el-dialog title="信号源配置" :visible.sync="dialogFormVisible2"  width="50%">
      <table>
        <tr class="ta-c">
          <td></td>
          <td>信号源名称</td>
        ...
        <tr v-for="(item,index) in liveConfig" :key="index"
        v-if="item.deleted === 0">
          <td>{{index+1}}</td>
          <td><el-input v-model="item.createUser" style="width: 200px;margin-right: 10px;"></el-input></td>
          <td><el-input v-model="item.liveName" style="width: 200px;margin-right: 10px;"></el-input></td>
          <td><el-input v-model="item.liveUrl"  style="width: 200px;margin-right: 10px;"></el-input></td>
          <td @click="deleteInfo(item)" v-if="!isAdd1"><img src="../../assets/image/delete-q.png"></td>
        </tr>
      </table>
      <div class="mt-20 ta-c" v-if="liveConfig.length === 0 && !isAdd1">暂无数据。。。</div>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    editMatchLiveConfig () {
      ...
      this.$axios('post', '/match/editMatchLiveConfig', param).then((res) => {
        if (res.code === 200) {
          this.$message({
            type: 'success',
            message: '配置成功!'
    ...
    getMatchLiveConfigList (id) {
      ...
      this.$axios('post', '/match/getMatchLiveConfigList', param).then((res) => {
        if (res.code === 200) {
    ...
    addSumbit () {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          this.$set(this.form, `startTime`, this.fromDate(this.form.startTime))
          let param = this.form
          this.$axios('post', '/match/addMatch', param).then((res) => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
                message: '新增成功!'
