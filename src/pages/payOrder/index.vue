    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <span v-if="scope.row.payChannelType === 1">三方或者四方支付</span>
          <span v-if="scope.row.payChannelType === 2">固码个人收款</span>
        </template>
      </el-table-column>
      <el-table-column
        label="充值状态"
  ...
  export default {
    ...
    methods: {
      getInfo () {
        ...
        this.$axios('post', '/recharge/list?pageNo=' + this.ps.pageNo + '&pageSize=' + this.ps.pageSize, param).then((res) => {
          if (res.code === '0') {
