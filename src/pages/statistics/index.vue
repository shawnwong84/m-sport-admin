<template>
  <div class="info-table">
    ...
    <el-dialog title="系统配置管理" :visible.sync="dialogFormVisible"  width="420px">
      <el-form :model="form" ref="form" :rules="rules">
        <el-form-item label="描述" :label-width="formLabelWidth" prop="desc" >
          <el-input v-model="form.desc" autocomplete="off" style="width: 238px;"></el-input>
        </el-form-item>
        <el-form-item label="参数名" :label-width="formLabelWidth" prop="name" v-if="isAdd">
          <el-input  v-model="form.name" autocomplete="off" style="width: 238px;"></el-input>
        </el-form-item>
        <el-form-item label="参数值" :label-width="formLabelWidth" prop="value" >
          <el-input type="textarea"
                    :rows="10" v-model="form.value" autocomplete="off" style="width: 238px;"></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="clear">取 消</el-button>
        <el-button type="primary" @click="addSumbit" v-if="isAdd">确 定</el-button>
        <el-button type="primary" @click="updateSumbit" v-if="isUdate">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 查询系统配置列表
    getInfo () {
      this.$axios('post', '/systemConfig/backGetConfigList').then((res) => {
        if (res.code === 200) {
    ...
    addSumbit () {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          let param = this.form
          this.$axios('post', '/systemConfig/saveOrUpdateConfig', param).then((res) => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
                message: '新增成功!'
