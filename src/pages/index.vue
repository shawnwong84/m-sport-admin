  ...
  <div class="layout">
    <el-container>
      <el-aside width="200px">
        <div class="logo"><img src="../assets/image/header-logo.png" width="80%"></div>
        <div class="left" :style="{height: scollerHeight}">
          <div v-for="(item,index) in menuList" :key="index">
            <div class="menu-list"
                 :class="routeName === item.jumpPath?'bg-bule':''">
              <div @click="toPage(item.jumpPath)" class="fx align-items">
                <div>
                  <img :src="require('../assets/image/'+item.jumpPath+'-h.png')" v-if="routeName === item.jumpPath" width="30px" />
                  <img :src="require('../assets/image/'+item.jumpPath+'-q.png')" v-else width="30px"/>
                </div>
                <div class="ml-10 ">{{item.name}}</div>
              </div>
            </div>
            <div class="menu-list" @click="toPage('role')" v-if="item.jumpPath === 'account' && showMenu"
                 :class="routeName === 'role'?'bg-bule':''">
              <div  style="padding-left: 40px;">角色管理</div>
            </div>
            <div class="menu-list" @click="toPage('column')" v-if="item.jumpPath === 'account' && showMenu"
      ...
      <el-container>
        <el-header>
          <div class="fx align-items head-right">
            <div class="ml-10"><img src="../assets/image/logo.jpg" class="head-img"/></div>
...
export default {
  ...
  mounted () {
    ...
    if (this.routeName === 'account' || this.routeName === 'role' || this.routeName === 'column') {
      this.showMenu = true
    }
    if (!Cookie.get('token')) {
